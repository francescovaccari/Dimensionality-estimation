function [crossval_PCA] = eval_num_PCs_cross_val(X, n_cross, n_dims_max, tol, max_iter, display)
...
c = cvpartition(size(X,1), 'KFold', n_cross);
...
for cross = 1:n_cross
    ...
    for k = 1:n_dims_max
        Z = X;
        mu = mean(Z);
        Z = bsxfun(@minus, Z, mu);  % Center the Z matrix (subtract mean)
